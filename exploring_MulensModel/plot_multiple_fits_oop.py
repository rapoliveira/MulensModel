...
class PlotMultipleModels(UlensModelFit):
    ...
    def __init__(
            self, photometry_files, model=None, model_2=None, model_3=None,
            plots=None, other_output=None
            ):
        ...
        self._list_of_models = [model, model_2, model_3]  # change yaml...

        self._read_data()
        self._check_plot_settings(**self._plot_settings)
        self._events = self._get_list_of_events()

        # self._set_default_parameters()  # check all the given parameters...
        print('\nPassed here!!!\n')
        breakpoint()
    ...
    def _read_data(self, test=None):
        ...
        self._event_data = mm.MulensData(my_dataset, phot_fmt='mag')
...
if __name__ == '__main__':
    ...
    # plot_multiple_models.generate_plot()

    # event_id, event_data = read_data(all_settings)  ## DONE!
    # plt_settings = all_settings['plots']['best model']
    # plt_settings['event_id'] = event_id  ## useless after oop...
    # models = [val for (key, val) in all_settings.items() if 'model' in key]
